/**********************************************************************
 * Widget:  jquery.widget.bootmodal.js                                *
...
(function ($) {
  $.widget("custom.bootmodal", {
    ...
    //
    templates: {
      modal: '<div class="modal" tabindex="-1" role="dialog" aria-hidden="true">' + '   <div class="modal-dialog">' + '       <div class="modal-content">' + '           <div class="modal-body">' + "           </div>" + "       </div>" + "   </div>" + "</div>",
      header: '<div class="modal-header">' + '   <h5 class="modal-title"></h5>' + "</div>",
      footer: '<div class="modal-footer">' + "</div>",
      btnEscape: '<button type="button" class="btn-escape close" aria-hidden="true">&times;</button>',
      btnModal: '<button type="button" class="btn btn-action"></button>',
    },
    /******************************/
    /*                            */
    /* PRIVAT METHODS             */
    ...
    _create: function () {
      ...
      if (typeof $.fn.modal === "undefined") {
        throw new Error('"$.fn.modal" is not defined; please double check you have included the Bootstrap JavaScript library. ' + "See https://getbootstrap.com/docs/4.6/getting-started/introduction/#js for more details.");
      }
      //
      //
    ...
    _buildModal: async function () {
      ...
      if (that.options.template) {
        await $.get(that.options.template + (that.options.template.search(/\?/) !== -1 ? "&" : "?") + "t=" + Date.now())
          .done(function (html) {
